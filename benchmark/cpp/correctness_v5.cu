...
bool run_correctness(bool causal) {
    ...
    auto set_attr_err = cudaFuncSetAttribute(
        forward::flash_attention_forward_v5_mma<Cfg>,
    ...
    forward::flash_attention_forward_v5_mma<Cfg><<<grid, block, smem>>>(args);
...
int main() {
    printf("v5 correctness test (non-causal)\n");
    bool ok = run_correctness(false);
    printf("v5 correctness: %s\n", ok ? "PASS" : "FAIL");
    return ok ? 0 : 1;
}
